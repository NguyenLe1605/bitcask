...
def check_extenson(x: str) -> bool:
    extensions = [".h", ".cpp", ".hpp"]
    return any(filter(lambda ext: x.endswith(ext), extensions))
...
def check(arguments, source_dir):
    ...
    for directory, subdirs, filenames in os.walk(source_dir):
        ...
        source_files = [x for x in fullpaths if check_extenson(x)]
        formatted_filenames.extend(
            ...
            [
                ...
                if not any(
                    (
                        fnmatch.fnmatch(filename, exclude_glob)
    ...
    if arguments.fix:
        ...
        if len(formatted_filenames) > 0:
            subprocess.check_call(
                [arguments.clang_format_binary, "-i"] + formatted_filenames
            )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("clang_format_binary", help="Path to the clang-format binary")
    parser.add_argument(
        "exclude_globs",
    ...
    parser.add_argument(
        "--source_dirs", help="Comma-separated root directories of the code"
    )
    parser.add_argument(
        "--fix",
        default=False,
